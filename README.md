...
## Поведение команд меню
- **Файл → Закрыть**: спрашивает, нужно ли сохранить изменения, затем очищает редактор.
- **Правка → Изменить**: открывает диалог редактирования выделенного факта/правила.

## Как вводить свои запросы
1. В приложении откройте **Вопросы → Произвольный запрос...**.
2. Вводите запрос **без** `?-` и **без** точки в конце.
   - Пример: `has_property(volvo_fh16, uses_roads)`
   - Пример: `member_of(yamaha_r1, transport)`
3. Результат появится в нижнем окне вывода.
